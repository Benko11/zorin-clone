...
export default function App() {
  ...
  const [searchEng, setSearchEng] = useState<number>(0);
  ...
  const switchEngine = (eng: number) => {
    setSearchEng(eng);
    searchRef.current.focus();
  };

  return (
    <div className="bg-gradient-to-b from-sky-500 to-cyan-800 h-screen">
      <div className="h-screen text-slate-50 flex flex-col gap-32 justify-center items-center -translate-y-12">
        <div className="flex gap-16">
          <div
            className={`font-bold text-[95px] cursor-pointer ${
              searchEng === 0 ? "text-slate-50" : "text-slate-400"
            }`}
            onClick={() => switchEngine(0)}
          >
            G
          </div>
          <svg
            role="img"
            className={`${
              searchEng === 1 ? "fill-slate-50" : "fill-slate-400"
            } w-20 cursor-pointer`}
            onClick={() => switchEngine(1)}
            viewBox="0 0 24 24"
